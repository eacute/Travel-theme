    ...
    $.fn.extend({
        jqslider: function (options) {
            ...
            return this.each(function () {
                ...
                var toolTipLeft = $('<div/>').addClass('jq-tooltip left');
                var toolTipRight = $('<div/>').addClass('jq-tooltip right');
                ...
                element.prepend(toolTipRight);
                element.prepend(toolTipLeft);
                var clear = $('<div></div>');
                ...
                function setTrackerPosition(cursorPosition) {
                    ...
                    if (cursorPosition >= sliderLeft && cursorPosition <= (sliderRight + 10)) {
                        if (trackerPosition >= 0 && trackerPosition <= sliderWidth + 4) {
                            ...
                            if (dragAmount > 0){
                            	if (settings.jq_append)  val=val+ settings.jq_append;
                            	if (currentHandle == 'left') toolTipLeft.html(val);
                                if (currentHandle == 'right') toolTipRight.html(val);
                            }  
                        ...
                        if (handle != null) {
                        	lpos= handle.position();
                        	rpos= handle.position();
                 			if (currentHandle == 'left') $ssize=toolTipLeft.width();
                 			if (currentHandle == 'right') $ssize = toolTipRight.width();
                 			$rsize= $ssize/2;
                            toolTipLeft.css('top', ( handle.height() +3));
                            toolTipRight.css('top', ( handle.height() +3));
                         
								 if (currentHandle == 'left') toolTipLeft.css('left', trackerPosition );
								 if (currentHandle == 'right') toolTipRight.css('left', trackerPosition  );
			
							
                          }

                        showToolTip = true;
                        $('.jq-tooltip.'+ currentHandle).fadeIn('slow');
